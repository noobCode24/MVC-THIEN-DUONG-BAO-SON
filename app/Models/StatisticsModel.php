...
class StatisticsModel extends Model
{
    ...
    {
        $data = $this->Select("SELECT * FROM bills WHERE bill_status = 1");
        return $data;
    }

    public function getTypeServiceBill() {
        $sql = "SELECT escate_id, COUNT(*) AS count
                FROM bills
                GROUP BY escate_id";
    ...
    public function getNameServices($id) {
        $sql = "SELECT escate_name from enterservice_category WHERE escate_id = $id";
        $data = $this->Select($sql);
        return $data;
    }

    public function getNameServicesBill() {
        $data = $this->getTypeServiceBill();
        foreach($data as  $key => $value) {
            $name = $this->getNameServices($value['escate_id']);
            $data[$key]['escate_name'] = $name[0]['escate_name'];
        }
        return $data;
    }
